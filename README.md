...
Install v0.3.1 beta with:

```bash
curl -fsSL https://raw.githubusercontent.com/kwonsyup/cross-agent-chat/v0.3.1/install.sh | sh
```
...
> Install the released `v0.3.1` tag, not an arbitrary PR. Identify active consumers and the
...
v0.3.1 is a macOS beta candidate. Claude Code uses its native cross-session mechanism. Codex
