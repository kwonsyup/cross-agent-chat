...
def test_peek_drains_full_frames_in_order_without_losing_remainder() -> None:
    ...
    for event_id in event_ids:
        courier.accept(event_id, "x" * 16000)
    ...
    frame_bytes = (json.dumps(frame, separators=(",", ":"), ensure_ascii=False) + "\n").encode()
    assert len(frame_bytes) <= MAX_FRAME_BYTES
    assert 0 < len(first) < len(event_ids)
    assert [item["event_id"] for item in first] == event_ids[: len(first)]

    courier.acknowledge([item["event_id"] for item in first])
    assert [item["event_id"] for item in courier.peek()] == event_ids[len(first) :]
