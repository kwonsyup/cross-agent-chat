...
def test_initialize_inside_a_batch_is_rejected_but_siblings_run(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
    monkeypatch.delenv("CROSS_AGENT_CHAT_PRESENCE", raising=False)
    batch = [
    ...
    assert array[1] == {"jsonrpc": "2.0", "id": 2, "result": {}}


def test_second_initialize_is_rejected(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
    monkeypatch.delenv("CROSS_AGENT_CHAT_PRESENCE", raising=False)
    monkeypatch.setattr(
...
def test_older_client_version_is_answered_with_the_supported_revision(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
    monkeypatch.delenv("CROSS_AGENT_CHAT_PRESENCE", raising=False)
    request = {
...
def test_notification_shaped_method_with_an_id_is_method_not_found(
    ...
    monkeypatch.setattr(
        "sys.stdin",
        _stdin(json.dumps({"jsonrpc": "2.0", "id": 1, "method": "notifications/cancelled"}) + "\n"),
    )
...
def test_tools_call_notification_has_no_effect_and_no_response(
    ...
    def fake_send(*_args: object) -> dict[str, object]:
    ...
    monkeypatch.setattr("sys.stdin", _stdin(json.dumps(notification) + "\n"))
...
def test_cancel_notification_cannot_recall_a_dispatched_effect(
    ...
    def record_send(*_args: object) -> dict[str, object]:
        sent.append("sent")
...
def test_execution_failure_after_a_possible_effect_is_a_tool_result(
    ...
    assert "uncertain" in responses[0]["result"]["content"][0]["text"]
...
def test_multibyte_oversized_frame_is_dropped_by_encoded_length(
    ...
    monkeypatch.delenv("CROSS_AGENT_CHAT_PRESENCE", raising=False)
    # 20000 four-byte characters: under the character bound, over the byte bound.
...
def test_single_request_without_handshake_still_works_for_integrated_hosts(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
    """Installed providers dispatch lone calls directly; that remains supported."""
    monkeypatch.delenv("CROSS_AGENT_CHAT_PRESENCE", raising=False)
    monkeypatch.setattr(
        "sys.stdin",
        _stdin(json.dumps({"jsonrpc": "2.0", "id": 1, "method": "tools/list"}) + "\n"),
    )
